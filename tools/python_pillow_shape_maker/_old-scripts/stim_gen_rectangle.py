...
sizes = np.linspace(.5,13,15)
shades = np.linspace(10,245,15)
    ...
    for i in range(dims.shape[0]):
        ...
        if (i+1) % 3 == 0:
            ...
            for ii in cluster_values:
                ...
                # draw rectangles
                x1 = canvas[0]/2 - d/2 - (d/2)*ii
                y1 = canvas[0]/2 + d/2 + (d/2)*ii
                x2 = canvas[0]/2 + d/2 + (d/2)*ii
                y2 = canvas[0]/2 - d/2 - (d/2)*ii
